...
void Graph::paintEvent(QPaintEvent *)
{
    ...
    if(s[0]=='r') // если r,то он рисует рандомный граф
    {
        ...
        {
            for(int v=1; v<n+1-v1;v++){
            mas[v1][v]=rand()%2+1;// через единицы и нули он проставляет,где есть ребра,а где нет
            }
        }
        for(int v1=1; v1<n+1; v1++)
        {
            for(int v=1; v<n+1-v1;v++){
            mas[v][v1]=mas[v1][v];// через единицы и нули он проставляет,где есть ребра,а где нет
            }
        }
    }
    else
    {
        ...
        fstream tOut;
        tOut.open("tr.txt", ios::in);
        tOut.seekg(n*n*(-1), ios::end);
        tOut >> s;
        for (int i=1 ; i!=n*n; i++)
        {
            if(s[i]=='1') // если ноль вершина есть
            {
                mas[i/n+1][i % n+1]=1;
            }
            else
...
if(1==1){
    ...
    ofstream myfile("tr.txt", std::ios::out);
    for(int i = 1; i<n+1; i++)
    {
        for(i1=1; i1<n+1; i1++)
        {
